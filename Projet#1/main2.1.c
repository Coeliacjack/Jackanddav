...
*/
int read_write_time(char read_or_write[]){

    Device device;
    int time_taken;
    char *device_name = program_call[ReadyQueue[0].command_num][ReadyQueue[0].process_num].Element_3;
    int size = program_call[ReadyQueue[0].command_num][ReadyQueue[0].process_num].data_size;
    ...
    if (strcmp(read_or_write, "read") == 0) {
        time_taken = (size*pow(10,6))/device.read_speed ;
    }
    else {
        time_taken = (size*pow(10,6))/device.write_speed ;
    }
    return time_taken;
}
...
Queue initilise_command_struct(char *process_name, int id) {
    Queue process;
    process.process_num = index_process(process_name);;
    process.command_num = 0;
    process.time_on_cpu = 0;
    process.parent_process_id = id;
    process.id = process_id_counter;
    process_id_counter++;
    return  process;
}

void add_running_proess(char *process_name, int id){ // This adds a program to the ready queue
    sys_time +=5;
    running_process +=1;
    ReadyQueue[0] = initilise_command_struct(process_name, id); // call add to ready queue
} 
void spawn(){
    add_running_proess(program_call[ReadyQueue[0].command_num][ReadyQueue[0].process_num].Element_3, ReadyQueue[0].id );
    ReadyQueue[0].child = 1;
    // call add to ready queue
}
void sleep()
{   
...
void Exit(){
   ...
   // find parent and set child to 0
}
void Blocked_IO(){
    BlockedQueue[0] = ReadyQueue[0];
}
void Wait(){
    if (ReadyQueue[0].child == 1){
...
void execute_system_call()
{
    ...
    if (strcmp(syscall, "spawn") == 0) {
        ...
        ReadyQueue[0].read_write_time = read_write_time(syscall);
        Blocked_IO();
...
void running()
{   
    add_running_proess(program_index[0], 0);
    CPU_Emulator();
    while (running_process != 0)
    {
    // sleep queue checked 
    // function to check if blocked funtions that are waiting are good to be added to the sleep queue
    // this will automatically be removed from the ready queue
    
    // reorder device queue
    // check all blocked cases
